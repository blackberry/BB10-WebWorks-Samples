...
var _notify = {
	...
	updateConfig: function (element) {
		...
		if (selected === 'basic') {
			...
			/* Invoke the picture viewer. */
			document.querySelector('#title').value = 'Picture Viewer';
			document.querySelector('#body').value = 'local:///img/cog.png';
			document.querySelector('#tag').value = 'config.custom.invoke';
			document.querySelector('#targetAction').value = 'bb.action.VIEW';
			document.querySelector('#payloadURI').value = 'local:///img/cog.png';
			document.querySelector('#payloadType').value = 'image/png';
		}
	},
